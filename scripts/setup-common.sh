...
function install_folly {
  ...
  cmake_install_dir folly -DBUILD_SHARED_LIBS="$VELOX_BUILD_SHARED" -DBUILD_TESTS=OFF -DFOLLY_HAVE_INT128_T=ON -DCMAKE_CXX_FLAGS="-fpermissive -march=native" 
}
...
function install_fbthrift {
  ...
  cmake_install_dir fbthrift -Denable_tests=OFF -DBUILD_TESTS=OFF -DBUILD_SHARED_LIBS=OFF -DCMAKE_CXX_FLAGS="-Wno-deprecated-declarations -O1 -g -march=native"
}
...
function install_faiss {
  if [[ $BUILD_FAISS == "true" ]]; then
    ...
    cmake_install_dir faiss \
      ...
      -DFAISS_ENABLE_BENCHMARKS=OFF -DCMAKE_CXX_FLAGS="-stdlib=libc++ -nostdinc++ -Xpreprocessor -fopenmp -I/usr/lib/llvm-11/lib/clang/11.0.1/include" \
      -DOpenMP_CXX_LIB_NAMES="omp" \
      -DOpenMP_CXX_FLAGS="-stdlib=libc++ -nostdinc++ -Xpreprocessor -fopenmp -I/usr/lib/llvm-11/lib/clang/11.0.1/include" \
